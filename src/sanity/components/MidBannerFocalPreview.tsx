...
const DEFAULT_Y: Record<DeviceKey, number> = { mobile: 50, tablet: 50, desktop: 50, xl: 50 }
...
export function MidBannerFocalPreview(props: ObjectInputProps) {
  ...
  const [mediaMode, setMediaMode] = useState<'images' | 'video'>(hasVideo ? 'video' : 'images')
  ...
  const hasPendingChanges = localY !== null || localX !== null
  ...
  function modeTabStyle(active: boolean): React.CSSProperties {
    return {
      ...
      padding: '10px 0',
      border: 'none',
      borderBottom: `3px solid ${active ? BRAND_YELLOW : 'transparent'}`,
      background: 'transparent',
      color: active ? BRAND_YELLOW : '#888',
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', color: '#1a1a1a' }}>
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #e0e0e0', marginBottom: 24 }}>
        <button type="button" style={modeTabStyle(mediaMode === 'images')} onClick={() => setMediaMode('images')}>
          📸 Still Images
        </button>
        <button type="button" style={modeTabStyle(mediaMode === 'video')} onClick={() => setMediaMode('video')}>
          🎬 Video {hasVideo && <span style={{ fontSize: 11, marginLeft: 4, color: BRAND_YELLOW }}>● active</span>}
        </button>
      </div>

      {/* ── VIDEO MODE ── */}
      {mediaMode === 'video' && videoMember && videoMember.kind === 'field' && (
        <div>
          <p style={{ fontSize: 12, color: '#666', marginBottom: 16 }}>
            Upload an MP4 or WebM. When set, the video plays on loop on the live site instead of the still images.
            Use the Still Images tab to upload per-device stills as a fallback.
          </p>
          ...
          {hasVideo && videoCdnUrl && (
            <div style={{ marginTop: 24 }}>
              <p style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#555', marginBottom: 12 }}>
                Video Crop Preview
              </p>
              <div style={{ display: 'flex', gap: 4, marginBottom: 16, flexWrap: 'wrap' }}>
                {DEVICES.map((d) => {
                  ...
                  return (
                    <button key={d.key} type="button" onClick={() => handleSwitchDevice(d.key)}
                      style={{ padding: '6px 16px', border: `2px solid ${isActive ? BRAND_YELLOW : '#ccc'}`, borderRadius: 6, background: isActive ? BRAND_YELLOW : '#fff', color: isActive ? '#fff' : '#333', fontWeight: isActive ? 700 : 400, cursor: 'pointer', fontSize: 13, transition: 'all 0.15s' }}
                    >
                      {d.label}
                    </button>
                  )
              ...
              <div style={{ width: device.previewW, height: device.previewH, overflow: 'hidden', borderRadius: 8, border: `1px solid ${BRAND_YELLOW}`, background: '#000', maxWidth: '100%' }}>
                <video src={videoCdnUrl} autoPlay muted loop playsInline
              ...
              <div style={{ marginTop: 16, marginBottom: 12 }}>
                ...
                <input type="range" min={0} max={100} step={5} value={displayY} onChange={(e) => setLocalY(Number(e.target.value))} style={{ width: '100%', accentColor: BRAND_YELLOW, cursor: 'pointer' }} />
              </div>
              <div style={{ marginBottom: 12 }}>
                ...
                <input type="range" min={0} max={100} step={5} value={displayX} onChange={(e) => setLocalX(Number(e.target.value))} style={{ width: '100%', accentColor: BRAND_YELLOW, cursor: 'pointer' }} />
              </div>
              {hasPendingChanges && (
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  <button type="button" onClick={handleSaveForDevice} style={{ padding: '8px 16px', background: BRAND_YELLOW, color: '#fff', border: 'none', borderRadius: 6, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>Save for {device.label}</button>
                  <button type="button" onClick={handleSaveForAll} style={{ padding: '8px 16px', background: '#333', color: '#fff', border: 'none', borderRadius: 6, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>Save for All Devices</button>
                </div>
              )}
            </div>
          )}
      ...
      {mediaMode === 'images' && (
        <div>
          {hasVideo && (
            <div style={{ marginBottom: 16, padding: '10px 14px', background: '#fffdf0', border: `1px solid ${BRAND_YELLOW}`, borderRadius: 6, fontSize: 12, color: '#b08a00' }}>
              A video is active on the live site. These still images are kept as a backup. Switch to the Video tab to manage your video.
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 20, flexWrap: 'wrap' }}>
            {DEVICES.map((d) => {
              const isActive = d.key === activeDevice
              const hasImage = Boolean((objectValue[d.imageField as keyof ObjectValue] as ObjectValue['imageMobile'])?.asset?._ref)
              return (
                <button key={d.key} type="button" onClick={() => handleSwitchDevice(d.key)}
                  style={{ padding: '6px 16px', border: `2px solid ${isActive ? BRAND_YELLOW : hasImage ? '#4caf50' : '#ccc'}`, borderRadius: 6, background: isActive ? BRAND_YELLOW : '#fff', color: isActive ? '#fff' : '#333', fontWeight: isActive ? 700 : 400, cursor: 'pointer', fontSize: 13, transition: 'all 0.15s' }}
                >
                  {d.label}{hasImage && !isActive && <span style={{ marginLeft: 6, color: '#4caf50', fontSize: 11 }}>✓</span>}
                </button>
              )
            })}
          </div>
          ...
          {imageUrl && (
            <>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ width: device.previewW, height: device.previewH, overflow: 'hidden', borderRadius: 8, border: `1px solid ${isUsingFallback ? '#f0d060' : '#ddd'}`, background: '#000', maxWidth: '100%' }}>
                  <img src={imageUrl} alt={`${device.label} crop preview`}
                    style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: `${displayX}% ${displayY}%`, display: 'block' }}
                  />
                </div>
              </div>
