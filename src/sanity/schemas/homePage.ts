      ...
      of: [{
        ...
        fields: [
          ...
          defineField({
            ...
            description: 'How many seconds to play before pausing. Default is 60 seconds (1 minute).',
            type: 'number',
            initialValue: 60,
            hidden: ({ parent }) => !((parent as { audio?: unknown })?.audio),
            group: 'settings',
          ...
          defineField({
            name: 'audioRepeatDelay',
            title: 'Repeat Delay (seconds)',
            description: 'How long to stay silent before replaying the snippet. Default is 60 seconds. The hero must still be on screen for it to replay.',
            type: 'number',
            initialValue: 60,
            hidden: ({ parent }) => !((parent as { audio?: unknown })?.audio),
            group: 'settings',
